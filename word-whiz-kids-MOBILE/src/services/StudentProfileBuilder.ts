...
// Mock AssessmentSystem for mobile if it doesn't exist, or just skip it
const AssessmentSystem = {
    getStudentStats: (_studentName: string) => {
        // Mobile might not have full assessment tracking yet, return empty stats
        return { total: 0, correct: 0, score: 0, byMode: {}, records: [] };
    }
};
    ...
    getStudentContext: (studentName: string, pin: string): string => {
        ...
        try {
            ...
            if (stats.total > 0) {
                ...
                Object.entries(stats.byMode).forEach(([mode, data]: [string, any]) => {
